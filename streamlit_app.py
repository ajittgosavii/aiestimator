...
# ==================== TAB 12: CONTROL CENTER - HUMAN OVERSIGHT ====================
with tab12:
    st.header("🎛️ Control Center - Human Oversight & Manual Controls")
    
    st.markdown("""
    **Centralized dashboard for human oversight, approvals, and manual control** of all AI agents and autonomous systems.
    This is your command center for reviewing AI decisions, managing approvals, and maintaining human-in-the-loop governance.
    """)
    ...
    # ==================== CONTROL TAB 1: APPROVAL QUEUE ====================
    with control_tab1:
        st.subheader("⏳ Pending Approvals")
        
        st.markdown("""
        **High-risk actions awaiting human approval** across all AI agents. Review, approve, or reject recommendations 
        with detailed context and risk assessment.
        """)
        
        # Approval queue metrics
        col1, col2, col3, col4, col5 = st.columns(5)
        with col1:
            st.metric("Pending Approvals", "23", "+5 today")
        with col2:
            st.metric("Avg Wait Time", "2.3 hrs", "-0.5 hrs")
        with col3:
            st.metric("Approved (24h)", "47", "94% approval rate")
        with col4:
            st.metric("Rejected (24h)", "3", "6% rejection rate")
        with col5:
            st.metric("Expired", "2", "Timeout after 24h")
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            approval_agent = st.selectbox("Filter by Agent", ["All Agents", "Security", "Compliance", "DevOps", "Database", "FinOps", "Policy"], key="approval_filter_agent")
        with col2:
            approval_priority = st.selectbox("Priority", ["All", "Critical", "High", "Medium", "Low"], key="approval_filter_priority")
        with col3:
            approval_category = st.selectbox("Category", ["All", "Security Changes", "Cost Commitments", "Policy Updates", "Access Grants", "Infrastructure Changes"], key="approval_filter_category")
        with col4:
            approval_age = st.selectbox("Age", ["All", "< 1 hour", "1-4 hours", "4-12 hours", "> 12 hours"], key="approval_filter_age")
        ...
        pending_approvals = [
            {
                "id": "APR-2024-00234",
                "agent": "💰 FinOps",
                "title": "Purchase 3-Year Reserved Instance Commitment",
                "details": "Commitment for 150 r5.4xlarge instances across us-east-1 and us-west-2",
                "savings": "$427,000/year",
                "cost": "$1,840,000 upfront",
                "risk": "High",
                "confidence": "96%",
                "submitted": "2 hours ago",
                "submitter": "finops-agent@system",
                "priority": "🔴 Critical"
            },
            {
                "id": "APR-2024-00233",
                "agent": "🛡️ Security",
                "title": "Modify Production Security Group Rules",
                "details": "Allow port 8443 from specific IP range for new monitoring system",
                "savings": "N/A",
                "cost": "No cost",
                "risk": "Medium",
                "confidence": "89%",
                "submitted": "4 hours ago",
                "submitter": "security-agent@system",
                "priority": "🟠 High"
            },
            {
                "id": "APR-2024-00232",
                "agent": "📋 Policy",
                "title": "Deploy New SCP to Production OU",
                "details": "Enforce IMDSv2 requirement across all production accounts",
                "savings": "Security enhancement",
                "cost": "No cost",
                "risk": "High",
                "confidence": "92%",
                "submitted": "5 hours ago",
                "submitter": "policy-agent@system",
                "priority": "🔴 Critical"
            },
            {
                "id": "APR-2024-00231",
                "agent": "🗄️ Database",
                "title": "Schema Modification on Production RDS",
                "details": "Add new composite index on transactions table (12M rows)",
                "savings": "35% query performance improvement",
                "cost": "~2 hour downtime window",
                "risk": "Medium",
                "confidence": "91%",
                "submitted": "7 hours ago",
                "submitter": "database-agent@system",
                "priority": "🟡 Medium"
            },
            {
                "id": "APR-2024-00230",
                ...
                "details": "Release v2.4.0 with security patches and feature updates",
                "savings": "N/A",
                "cost": "15 min deployment",
                "risk": "Medium",
                "confidence": "94%",
                "submitted": "8 hours ago",
                "submitter": "devops-agent@system",
                "priority": "🟠 High"
            }
        ]
        
        for idx, approval in enumerate(pending_approvals):
            risk_color = {"High": "#EF4444", "Medium": "#F59E0B", "Low": "#22C55E"}[approval["risk"]]
            priority_color = "🔴" if "Critical" in approval["priority"] else "🟠" if "High" in approval["priority"] else "🟡"
            
            with st.expander(f"{approval['priority']} {approval['id']} - {approval['title']}", expanded=(idx==0)):
                col1, col2 = st.columns([2, 1])
                ...
                with col1:
                    ...
                    **Submitted:** {approval['submitted']} by {approval['submitter']}  
                    **AI Confidence:** {approval['confidence']}
                    """)
                    
                    if approval['savings'] != "N/A":
                        st.markdown(f"**💰 Expected Savings:** {approval['savings']}")
                    
                    st.markdown(f"**💳 Cost/Impact:** {approval['cost']}")
                    st.markdown(f"**⚠️ Risk Level:** <span style='color: {risk_color}; font-weight: bold;'>{approval['risk']}</span>", unsafe_allow_html=True)
                
                with col2:
                    st.markdown("#### AI Reasoning")
                    st.info(f"""
                    **Why this recommendation:**
                    - Pattern analysis: 30 days historical data
                    - Risk assessment: Automated + manual review
                    - Business alignment: Approved strategy
                    - Compliance check: ✅ Passed
                    
                    **Recommendation:** {'APPROVE' if approval['confidence'] > '90%' else 'REVIEW'}
                    """)
                ...
                with col1:
                    if st.button(f"✅ Approve", key=f"approve_{approval['id']}", type="primary", use_container_width=True):
                        st.success(f"✅ {approval['id']} APPROVED - Action will execute in 5 minutes")
                        st.balloons()
                
                with col2:
                    if st.button(f"❌ Reject", key=f"reject_{approval['id']}", use_container_width=True):
                        st.error(f"❌ {approval['id']} REJECTED - Notification sent to submitter")
                
                with col3:
                    if st.button(f"⏸️ Defer", key=f"defer_{approval['id']}", use_container_width=True):
                        st.warning(f"⏸️ {approval['id']} DEFERRED - Will expire in 24 hours")
                
                with col4:
                    if st.button(f"📝 Request More Info", key=f"info_{approval['id']}", use_container_width=True):
                        st.info(f"📝 Information request sent to agent owner")
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            if st.button("✅ Approve All Low Risk (< $5K)", key="bulk_approve_low_risk", use_container_width=True):
                st.success("✅ Approved 7 low-risk items")
        with col2:
            if st.button("📧 Notify All Pending > 12h", key="bulk_notify_pending", use_container_width=True):
                st.info("📧 Sent reminders for 3 pending approvals")
        with col3:
            if st.button("📥 Export Approval Queue", key="export_approval_queue", use_container_width=True):
                st.success("✅ Downloaded pending_approvals.csv")
    
    # ==================== CONTROL TAB 2: OVERRIDE CONTROLS ====================
    with control_tab2:
        st.subheader("🎚️ Manual Override & Emergency Controls")
        
        st.markdown("""
        **Master controls for all AI agents and autonomous systems.** Use these controls to pause, resume, 
        or completely disable autonomous operations when needed.
        """)
        ...
        with col1:
            st.markdown("### 🚨 Global Emergency Controls")
            
            st.markdown("""
            <div style='background: linear-gradient(135deg, #7F1D1D 0%, #991B1B 100%); padding: 1.5rem; border-radius: 10px; border: 2px solid #DC2626; margin: 1rem 0;'>
                <h4 style='color: #FEF2F2; margin-top: 0;'>⚠️ EMERGENCY STOP</h4>
                <p style='color: #FCA5A5; margin-bottom: 1rem;'>
                    Immediately halt ALL autonomous operations across all 6 AI agents. 
                    Use only in emergency situations. All pending actions will be cancelled.
                </p>
            </div>
            """, unsafe_allow_html=True)
            
            emergency_confirm = st.checkbox("⚠️ I understand this will stop all automated operations", key="emergency_confirm_checkbox")
            
            col_a, col_b = st.columns(2)
            with col_a:
                if st.button("🛑 EMERGENCY STOP ALL", key="emergency_stop_all", disabled=not emergency_confirm, type="primary", use_container_width=True):
                    st.error("🛑 EMERGENCY STOP ACTIVATED - All agents paused!")
                    st.warning("System status: Manual mode only")
            
            with col_b:
                if st.button("▶️ Resume All Operations", key="resume_all_operations", use_container_width=True):
                    st.success("✅ All agents resumed - Autonomous operations restored")
        
        with col2:
            st.markdown("### 📊 System Status")
            st.metric("Active Agents", "6 / 6", "All operational")
            st.metric("Paused Actions", "0", "None")
            st.metric("Manual Mode", "Disabled", "Auto mode active")
        ...
        st.markdown("**Enable/disable autonomous capabilities for each agent independently**")
        ...
        agents_config = [
            {
                ...
                "actions_today": 156,
                "auto_enabled": True,
                "description": "Autonomous security remediation"
            },
            {
                "name": "⚖️ Compliance Agent",
                "status": "Active",
                "actions_today": 89,
                "auto_enabled": True,
                "description": "Automatic compliance fixes"
            },
            {
                "name": "⚙️ DevOps Agent",
                "status": "Active",
            ...
            },
            {
                ...
                "actions_today": 67,
                "auto_enabled": True,
                "description": "Database access management"
            },
            {
                "name": "💰 FinOps Agent",
                "status": "Active",
        ...
        for idx, agent in enumerate(agents_config):
            status_color = "#22C55E" if agent["status"] == "Active" else "#EF4444"
            ...
            <div style='background: #2E3440; padding: 1rem; border-radius: 8px; margin: 0.5rem 0; border-left: 4px solid {status_color};'>
                <div style='display: flex; justify-content: space-between; align-items: center;'>
                    <div>
                        ...
                        <span style='color: {status_color}; margin-left: 1rem;'>● {agent['status']}</span>
                    </div>
                    <div style='text-align: right;'>
                        <span style='color: #88C0D0;'>Actions today: </span>
                        <strong style='color: #A3BE8C;'>{agent['actions_today']}</strong>
                    </div>
                </div>
                <p style='margin: 0.5rem 0 0 0; color: #D8DEE9; font-size: 0.9rem;'>{agent['description']}</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            with col1:
                auto_mode = st.toggle(f"Auto Mode", value=agent["auto_enabled"], key=f"auto_{idx}")
            
            with col2:
                if st.button("⏸️ Pause", key=f"pause_{idx}", use_container_width=True):
                    st.warning(f"⏸️ {agent['name']} paused")
            
            with col3:
                if st.button("▶️ Resume", key=f"resume_{idx}", use_container_width=True):
                    st.success(f"▶️ {agent['name']} resumed")
            
            with col4:
                if st.button("📊 View Logs", key=f"logs_{idx}", use_container_width=True):
                    st.info(f"Opening logs for {agent['name']}")
            
            st.markdown("---")
        
        # Rate limiting controls
        st.markdown("### ⚡ Rate Limiting & Safety Controls")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("#### Global Rate Limits")
            max_actions_hour = st.slider("Max actions per agent per hour", 0, 1000, 100, key="override_max_actions_hour")
            max_cost_action = st.slider("Max cost per single action ($)", 0, 50000, 5000, step=1000, key="override_max_cost_action")
            max_resources_batch = st.slider("Max resources per batch operation", 1, 100, 20, key="override_max_resources_batch")
            
            st.checkbox("Pause on 3+ failures in 1 hour", value=True, key="override_pause_on_failures")
            st.checkbox("Require confirmation for production changes", value=True, key="override_confirm_prod_changes")
            st.checkbox("Enable audit logging for all actions", value=True, key="override_enable_audit_logging")
        
        with col2:
            st.markdown("#### Notification Settings")
            st.checkbox("Email on emergency stop", value=True, key="override_email_emergency_stop")
            st.checkbox("Slack alert on agent pause", value=True, key="override_slack_agent_pause")
            st.checkbox("PagerDuty for critical failures", value=True, key="override_pagerduty_critical")
            st.checkbox("Daily summary report", value=True, key="override_daily_summary")
            
            notification_email = st.text_input("Notification email", value="cloud-ops@company.com", key="override_notification_email")
            slack_channel = st.text_input("Slack channel", value="#cloud-ops-alerts", key="override_slack_channel")
        
        col1, col2 = st.columns(2)
        with col1:
            if st.button("💾 Save Settings", key="save_override_settings", type="primary", use_container_width=True):
                st.success("✅ Override settings saved successfully")
        with col2:
            if st.button("↩️ Reset to Defaults", key="reset_override_defaults", use_container_width=True):
                st.info("🔄 Settings reset to defaults")
    
    # ==================== CONTROL TAB 3: AI DECISION REVIEW ====================
    with control_tab3:
        st.subheader("🔍 AI Decision Review & Quality Assurance")
        
        st.markdown("""
        **Review recent autonomous decisions** made by AI agents. Flag incorrect decisions, 
        provide feedback, and improve AI performance through human oversight.
        """)
        
        # Review metrics
        col1, col2, col3, col4, col5 = st.columns(5)
        with col1:
            st.metric("Actions (24h)", "1,169", "+12%")
        with col2:
            st.metric("Accuracy Rate", "98.7%", "+0.3%")
        with col3:
            st.metric("Flagged for Review", "15", "1.3%")
        with col4:
            st.metric("Rollbacks (7d)", "3", "-2")
        with col5:
            st.metric("Avg Confidence", "94.2%", "+1.1%")
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            review_agent = st.selectbox("Agent", ["All Agents", "Security", "Compliance", "DevOps", "Database", "FinOps", "Policy"], key="review_agent_filter")
        with col2:
            review_outcome = st.selectbox("Outcome", ["All", "Success", "Failed", "Rolled Back", "Flagged"], key="review_outcome_filter")
        with col3:
            review_confidence = st.selectbox("Confidence", ["All", ">95%", "90-95%", "85-90%", "<85%"], key="review_confidence_filter")
        with col4:
            review_timeframe = st.selectbox("Timeframe", ["Last 24 hours", "Last 7 days", "Last 30 days", "Custom"], key="review_timeframe_filter")
        ...
        recent_decisions = [
            {
                "timestamp": "2024-11-24 14:47:23",
                "agent": "💰 FinOps",
                "action": "Terminated 3 idle EC2 instances",
                "resources": "i-0abc123, i-0def456, i-0ghi789",
                "confidence": "97%",
                "impact": "$847/month saved",
                "outcome": "✅ Success",
            ...
            },
            {
                ...
                "resources": "s3://prod-data-analytics",
                "confidence": "99%",
                "impact": "Critical security risk mitigated",
                "outcome": "✅ Success",
                "review_status": "✅ Approved"
            },
            {
                "timestamp": "2024-11-24 14:18:07",
                "agent": "⚖️ Compliance",
                "action": "Enabled encryption on RDS instance",
                "resources": "rds:prod-payments-db",
                "confidence": "96%",
                "impact": "PCI DSS compliance restored",
                "outcome": "✅ Success",
                "review_status": "Not reviewed"
            },
            {
                "timestamp": "2024-11-24 13:55:41",
                "agent": "⚙️ DevOps",
                "action": "Auto-fixed failing build pipeline",
                "resources": "Pipeline: backend-api-main",
                "confidence": "91%",
                "impact": "15min downtime prevented",
                "outcome": "✅ Success",
                "review_status": "⚠️ Flagged"
            },
            {
                "timestamp": "2024-11-24 13:22:19",
                "agent": "🗄️ Database",
                "action": "Granted temp read-only access",
                "resources": "User: john.doe@company.com",
                "confidence": "94%",
                "impact": "4-hour access granted",
                "outcome": "✅ Success",
                "review_status": "Not reviewed"
            },
            {
                "timestamp": "2024-11-24 12:48:33",
                "agent": "💰 FinOps",
                "action": "Applied S3 lifecycle policy",
                "resources": "s3://archive-logs-2024",
                "confidence": "98%",
                "impact": "$234/month saved",
                "outcome": "✅ Success",
                "review_status": "Not reviewed"
            }
        ]
        ...
        for idx, decision in enumerate(recent_decisions):
            review_color = {"Not reviewed": "#64748B", "✅ Approved": "#22C55E", "⚠️ Flagged": "#F59E0B", "❌ Incorrect": "#EF4444"}[decision["review_status"]]
            
            with st.expander(f"{decision['timestamp']} - {decision['agent']} - {decision['action']}", expanded=(idx<2)):
                col1, col2 = st.columns([2, 1])
                ...
                with col1:
                    ...
                    **Review Status:** <span style='color: {review_color}; font-weight: bold;'>{decision['review_status']}</span>
                    """, unsafe_allow_html=True)
                ...
                with col1:
                    if st.button("✅ Approve Decision", key=f"approve_dec_{idx}", use_container_width=True):
                        st.success("✅ Decision approved - Added to training data")
                
                with col2:
                    if st.button("⚠️ Flag for Review", key=f"flag_dec_{idx}", use_container_width=True):
                        st.warning("⚠️ Flagged for management review")
                
                with col3:
                    if st.button("❌ Mark Incorrect", key=f"incorrect_dec_{idx}", use_container_width=True):
                        st.error("❌ Marked incorrect - Will improve future decisions")
                
                with col4:
                    if st.button("↩️ Rollback", key=f"rollback_dec_{idx}", use_container_width=True):
                        st.warning("↩️ Initiating rollback procedure...")
        
        st.markdown("---")
        
        # Feedback and improvement
        st.markdown("### 💬 Provide Feedback to Improve AI")
        ...
        with col1:
            ...
            feedback_text = st.text_area("Detailed Feedback", 
                placeholder="Describe what went wrong or how the AI could improve...",
                height=100,
                key="feedback_text_area")
            
            if st.button("📤 Submit Feedback", type="primary", key="submit_feedback_button"):
                st.success("✅ Feedback submitted - Will be used to retrain models")
        
        with col2:
            st.markdown("#### Feedback Impact")
            st.metric("Feedback Items", "234", "This month")
            st.metric("Model Improvements", "12", "From feedback")
            st.metric("Accuracy Gain", "+2.3%", "Since last retrain")
    
    # ==================== CONTROL TAB 4: ESCALATIONS ====================
    with control_tab4:
        st.subheader("⚠️ Escalations & Human Judgment Required")
        
        st.markdown("""
        **Items that require human judgment** - situations where the AI is uncertain, 
        conflicting policies exist, or anomalies are detected.
        """)
        
        # Escalation metrics
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Open Escalations", "8", "+2 today")
        with col2:
            st.metric("Avg Resolution Time", "3.2 hrs", "-0.8 hrs")
        with col3:
            st.metric("Escalation Rate", "0.7%", "Of all actions")
        with col4:
            st.metric("Resolved (7d)", "47", "100% resolved")
        ...
        escalations = [
            {
                ...
                "severity": "🔴 Critical",
                "agent": "💰 FinOps",
                "title": "Conflicting Cost Optimization Recommendations",
                "description": "Two agents recommend different actions for same resource set",
                "details": """
                **Conflict:**
                - FinOps Agent: Recommends rightsizing 50 instances → $45K/month savings
                - Security Agent: Recommends keeping current size for security buffer
                
                **Resource:** prod-api-cluster (50 instances)
                
                **Human Decision Needed:** Which recommendation takes priority?
                """,
                "age": "2 hours",
                "impact": "Medium"
            },
            {
                ...
                "description": "Unusual API calls from service account - unable to determine if legitimate",
                "details": """
                **Pattern:**
                - Service account: svc-analytics-prod
                - Unusual activity: 10,000+ ListBuckets calls in 1 hour
                - Source: us-west-2 (normal region)
                - Time: Outside business hours (2 AM PST)
                
                **AI Uncertainty:** Could be legitimate batch job or potential compromise
                
                **Human Decision Needed:** Investigate or auto-block?
                """,
                "age": "4 hours",
                "impact": "High"
            },
            {
                ...
                "description": "Dev team requests exception to production tagging policy",
                "details": """
                **Request:**
                - Team: Data Science Team
                - Exception: Skip 'CostCenter' tag for experimental ML instances
                - Duration: 90 days
                - Reason: Rapid experimentation, unknown cost center allocation
                
                **AI Recommendation:** DENY (violates org policy)
                
                **Human Decision Needed:** Grant exception or deny?
                """,
                "age": "6 hours",
                "impact": "Low"
            }
        ...
        for esc in escalations:
            severity_color = {"🔴 Critical": "#EF4444", "🟠 High": "#F59E0B", "🟡 Medium": "#FBBF24"}[esc["severity"]]
            
            with st.expander(f"{esc['severity']} {esc['id']} - {esc['title']}", expanded=True):
                col1, col2 = st.columns([2, 1])
                ...
                with col2:
                    ...
                    st.metric("Priority", esc['severity'])
                    st.metric("Age", esc['age'])
                    st.metric("Impact", esc['impact'])
                ...
                with col1:
                    if st.button("✅ Resolve", key=f"resolve_{esc['id']}", type="primary", use_container_width=True):
                        st.success(f"✅ {esc['id']} marked as resolved")
                
                with col2:
                    if st.button("📝 Add Notes", key=f"notes_{esc['id']}", use_container_width=True):
                        st.info("📝 Note editor opened")
                
                with col3:
                    if st.button("👥 Assign", key=f"assign_{esc['id']}", use_container_width=True):
                        st.info("👥 Assignment dialog opened")
                
                with col4:
                    if st.button("⬆️ Escalate Higher", key=f"escalate_{esc['id']}", use_container_width=True):
                        st.warning("⬆️ Escalated to leadership")
        
        st.markdown("---")
        
        # Escalation settings
        st.markdown("### ⚙️ Escalation Triggers")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("**Auto-escalate when:**")
            st.checkbox("AI confidence < 85%", value=True, key="escalate_low_confidence")
            st.checkbox("Conflicting recommendations", value=True, key="escalate_conflicts")
            st.checkbox("Cost impact > $10,000", value=True, key="escalate_high_cost")
            st.checkbox("Security risk score > 7/10", value=True, key="escalate_high_risk")
            st.checkbox("Policy conflict detected", value=True, key="escalate_policy_conflict")
        
        with col2:
            st.markdown("**Notification channels:**")
            st.checkbox("Email to on-call engineer", value=True, key="escalate_email_oncall")
            st.checkbox("Slack #escalations channel", value=True, key="escalate_slack_channel")
            st.checkbox("PagerDuty (critical only)", value=True, key="escalate_pagerduty")
            
            escalation_email = st.text_input("Escalation email", value="cloud-ops-escalations@company.com", key="escalation_email_input")
    
    # ==================== CONTROL TAB 5: AUDIT & COMPLIANCE ====================
    with control_tab5:
        st.subheader("📊 Audit Trail & Compliance Reporting")
        
        st.markdown("""
        **Complete audit trail** of all human approvals, overrides, and AI decisions 
        for compliance, governance, and audit purposes.
        """)
        
        # Audit metrics
        col1, col2, col3, col4, col5 = st.columns(5)
        with col1:
            st.metric("Total Actions (30d)", "34,567", "+8%")
        with col2:
            st.metric("Human Approved", "1,234", "3.6% of total")
        with col3:
            st.metric("Auto Executed", "33,333", "96.4% of total")
        with col4:
            st.metric("Rejected", "89", "0.3% of total")
        with col5:
            st.metric("Rolled Back", "12", "0.03% of total")
        ...
        col1, col2, col3, col4, col5 = st.columns(5)
        with col1:
            audit_type = st.selectbox("Action Type", ["All", "Approval", "Override", "AI Decision", "Rollback", "Escalation"], key="audit_type_filter")
        with col2:
            audit_user = st.selectbox("User", ["All Users", "john.doe@company.com", "jane.smith@company.com", "System"], key="audit_user_filter")
        with col3:
            audit_agent = st.selectbox("Agent", ["All Agents", "Security", "Compliance", "DevOps", "Database", "FinOps", "Policy"], key="audit_agent_filter")
        with col4:
            audit_outcome = st.selectbox("Outcome", ["All", "Approved", "Rejected", "Success", "Failed"], key="audit_outcome_filter")
        with col5:
            audit_date = st.date_input("Date Range", key="audit_date_filter")
        ...
        audit_entries = []
        users = ["john.doe@company.com", "jane.smith@company.com", "system@automated", "admin@company.com"]
        actions = ["Approved RI purchase", "Rejected security group change", "Paused FinOps agent", 
                  "Resumed DevOps agent", "Manually blocked S3 bucket", "Rolled back instance rightsizing",
                  "Escalated policy conflict", "Resolved escalation ESC-2024-00089"]
        
        for i in range(50):
            audit_entries.append({
                "Timestamp": (datetime.now() - timedelta(minutes=random.randint(1, 43200))).strftime("%Y-%m-%d %H:%M:%S"),
                "User": random.choice(users),
                "Action Type": random.choice(["Approval", "Override", "AI Decision", "Rollback", "Escalation"]),
                "Action": random.choice(actions),
                "Agent": random.choice(["Security", "Compliance", "DevOps", "Database", "FinOps", "Policy"]),
                "Resource": f"res-{random.randint(10000, 99999)}",
                "Outcome": random.choice(["✅ Success", "✅ Approved", "❌ Rejected", "⚠️ Warning"]),
                "Impact": f"${random.randint(100, 10000)}"
            })
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            if st.button("📥 Export CSV", key="export_audit_csv", use_container_width=True):
                st.success("✅ Downloaded audit_log.csv")
        with col2:
            if st.button("📊 Generate Report", key="generate_audit_report", use_container_width=True):
                st.success("✅ Compliance report generated")
        with col3:
            if st.button("📧 Email Report", key="email_audit_report", use_container_width=True):
                st.success("✅ Report sent to compliance@company.com")
        with col4:
            if st.button("🔍 Advanced Search", key="advanced_audit_search", use_container_width=True):
                st.info("🔍 Advanced search dialog opened")
        ...
        with col1:
            st.markdown("#### Approval Compliance")
            
            # Approval compliance chart
            approval_data = pd.DataFrame({
                "Category": ["Approved", "Auto-Executed", "Rejected", "Expired"],
                "Count": [1234, 33333, 89, 12],
                "Percentage": [3.6, 96.4, 0.3, 0.03]
            })
            
            fig = go.Figure(data=[go.Pie(
                labels=approval_data['Category'],
                values=approval_data['Count'],
                hole=0.4,
                marker_colors=['#22C55E', '#3B82F6', '#EF4444', '#F59E0B']
            )])
        ...
        with col2:
            st.markdown("#### Action Distribution by Agent")
            
            agent_actions = pd.DataFrame({
                "Agent": ["FinOps", "Security", "DevOps", "Compliance", "Database", "Policy"],
                "Actions": [12450, 8920, 6780, 4560, 1890, 967]
            })
            
            fig = go.Figure(data=[go.Bar(
                x=agent_actions['Agent'],
                y=agent_actions['Actions'],
                marker_color=['#8B5CF6', '#3B82F6', '#10B981', '#F59E0B', '#06B6D4', '#EC4899'],
                text=agent_actions['Actions'],
                textposition='outside'
            )])
        ...
        # SLA compliance
        st.markdown("#### ⏱️ Approval SLA Compliance")
        
        sla_data = pd.DataFrame({
            "Metric": ["< 1 hour", "1-4 hours", "4-12 hours", "> 12 hours"],
            "Count": [456, 567, 189, 22],
            "Target": [50, 40, 8, 2],
            "Actual %": [37.0, 46.0, 15.3, 1.8]
        })
        
        st.dataframe(sla_data, use_container_width=True, hide_index=True)
    
    # ==================== CONTROL TAB 6: SETTINGS ====================
    with control_tab6:
        st.subheader("⚙️ Control Center Settings")
        
        st.markdown("""
        **Configure approval workflows, notification preferences, and control center behavior.**
        """)
        ...
        with col1:
            ...
            st.checkbox("Email on pending approval", value=True, key="settings_email_pending")
            st.checkbox("Email on escalation", value=True, key="settings_email_escalation")
            st.checkbox("Email on emergency stop", value=True, key="settings_email_emergency")
            st.checkbox("Daily summary email", value=True, key="settings_email_daily")
            
            st.text_input("Primary email", value="cloud-ops@company.com", key="settings_primary_email")
            st.text_input("Escalation email", value="cloud-ops-leads@company.com", key="settings_escalation_email")
            
            st.markdown("### 📱 Slack Integration")
            
            st.checkbox("Slack notifications enabled", value=True, key="settings_slack_enabled")
            st.text_input("Approval channel", value="#cloud-ops-approvals", key="settings_slack_approval")
            st.text_input("Escalation channel", value="#cloud-ops-escalations", key="settings_slack_escalation")
            st.text_input("General alerts channel", value="#cloud-ops-alerts", key="settings_slack_alerts")
        
        with col2:
            st.markdown("### ⏱️ Approval Workflow Settings")
            
            st.number_input("Approval timeout (hours)", value=24, min_value=1, max_value=168, key="settings_approval_timeout")
            st.number_input("Auto-approve threshold ($)", value=5000, step=1000, key="settings_auto_approve_threshold")
            st.number_input("Escalation threshold (confidence %)", value=85, min_value=0, max_value=100, key="settings_escalation_threshold")
            
            st.checkbox("Require dual approval for >$50K", value=True, key="settings_dual_approval")
            st.checkbox("Require CISO approval for security changes", value=True, key="settings_ciso_approval")
            st.checkbox("Require CFO approval for >$200K commitments", value=True, key="settings_cfo_approval")
            
            st.markdown("### 🎯 Auto-Approval Rules")
            
            st.checkbox("Auto-approve low-risk, high-confidence actions", value=True, key="settings_auto_approve_enabled")
            st.slider("Confidence threshold for auto-approval", 0, 100, 95, format="%d%%", key="settings_confidence_threshold")
            st.slider("Max cost for auto-approval ($)", 0, 10000, 5000, step=500, key="settings_max_cost_auto")
        
        st.markdown("---")
        
        col1, col2, col3 = st.columns(3)
        with col1:
            if st.button("💾 Save All Settings", key="save_control_center_settings", type="primary", use_container_width=True):
                st.success("✅ All settings saved successfully")
        with col2:
            if st.button("↩️ Reset to Defaults", key="reset_control_center_defaults", use_container_width=True):
                st.warning("⚠️ Settings reset to default values")
        with col3:
            if st.button("📥 Export Configuration", key="export_control_center_config", use_container_width=True):
                st.success("✅ Downloaded control_center_config.json")
